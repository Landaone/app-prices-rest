...
**11. A count or enumeration presented at an approval gate is mechanical command output, never a recalled figure (sixteenth revision) — the correct half of an idea raised and initially declined.** Early in this change's own retrospective, a proposal to give each checkpoint a hand-maintained `checkpoints/*.md` validation file was declined as canonical-source duplication — correct for `Allowed modifications` (design D-Z, parts 1-2), which already belongs to the phase file. But a live pilot then hit the specific failure that proposal was actually reaching for: `ADOPT-03`'s approval gate presented a file count derived from the agent's own summarized recollection, and it was wrong three times in a row on the same gate (`docs/` 8 vs. the actual 7, `skills/` 14 vs. the actual 10, `ai-specs/agents/` 4 vs. the actual 3) — echoing the original pilot's own recorded miscounts at this exact step and at `ADOPT-09`/`ADOPT-10`. The fix is not a second, hand-maintained source of truth; `ADOPT-03`'s own `Validation` section already runs the correct mechanical `find` commands — **before** this revision, only *after* the copy. `00-conventions.md`'s evidence discipline gains an explicit, general rule: a count or enumeration presented at any approval gate is the literal output of a mechanical command run against the actual source before that gate is shown, never a recalled or summarized figure. `ADOPT-03` runs its existing `find` commands before presenting its gate, not only after, and quotes their output directly.

**12. `ADOPT-03`'s import payload is `<SPECBOOT_SOURCE>/packages/specboot/template/`, never `<SPECBOOT_SOURCE>`'s own root — a distinction the fifteenth revision's fix did not make, and a live pilot run exposed the gap (seventeenth revision).** `ADOPT-00`'s three-artifact validation (`SPECBOOT_ADOPTION_GUIDE.md`, `specboot-adoption/`, the orchestration skill) validates the bare source root — it says nothing about whether that same root's `docs/`/`ai-specs/` are free of the source repository's *own* project-specific content. Where the canonical source doubles as a real, self-adopted repository — exactly the case in this project, where the pinned commit's bare root carries the *Price API application's own* adopted `docs/`/`ai-specs/`, not a generic template — copying from the root would import that unrelated content into a fresh target, the template-contamination failure `ADOPT-06` exists to *find*, not one this step should be *causing*. The generic, redistributable payload lives at a separate, deliberately generic location: `packages/specboot/template/`. `01-prerequisites-and-install.md`'s `ADOPT-03` now imports from that path explicitly, extracted via `git archive` against the pinned commit where the source is a sparse checkout that does not materialize `packages/` on disk — the same technique the reference experiment already used and this design already documents elsewhere, now stated at the point `ADOPT-03` needs it rather than left to be rediscovered.

*Alternative rejected:* leave `<SPECBOOT_SOURCE>` as a single value serving both `ADOPT-00` and `ADOPT-03`, and rely on the operator or executing session to infer the payload subpath. Already tried, in effect: the fifteenth revision's fix correctly stopped `ADOPT-03` from re-deriving *which directory* is the source, but said nothing about *which subpath within* that directory is safe to copy — a live run reached exactly that gap immediately after the first fix closed the one before it.

*Alternative rejected:* a hand-maintained per-step validation file recording expected counts. This is the proposal declined at the top of this part, and the reason holds even now that the underlying problem is confirmed real: a hand-maintained count file drifts from the template the moment either changes without the other being updated, recreating exactly the duplicated-canonical-source risk `D-A` and `D-L` exist to prevent. Requiring the *existing* validation commands to run before the gate, not only after, gets the same guarantee — a human approves what a command printed — without a second file to keep in sync.

*Alternative rejected, for parts 6-10 (composing with parts 11 and 12 as the sixth and seventh instances of the same underlying principle — a live-verifiable claim beats a recalled one, whether the claim is an approval decision, a plain count, or a payload location):* leave these as documented rules and trust execution-time judgment to apply them correctly. Already tried, in the tenth revision's own text for parts 6 and 8 — stating the reasoning in `design.md` without a corresponding task or a tightened instruction in the phase file is exactly what let the gap survive one revision uncorrected.

**13. `ADOPT-04`'s install gate gains the same skip-when-already-satisfied treatment part 6 already gave `ADOPT-02`, closing a gap the eighteenth revision's own `Allowed modifications` sweep did not reach (nineteenth revision).** `02-codegraph.md`'s `ADOPT-04` bundles two actions under one gate worded "before software installation": following the official installation procedure to put `codegraph` on `PATH`, and then running `codegraph init` — a repository-local index build with no software installed at all, never what the gate names in the first place. A completed pilot run (`app-prices-rest-specboot-ai-adoption-v2`, `.specboot/adoption/ADOPTION-RUN-LOG.md`, `ADOPT-04` evidence block) already found CodeGraph on `PATH` from before the adoption began and recorded, correctly, that "no new installation performed, so `ADOPT-04`'s approval gate was never reached" — but that was the executing agent's own interpretation of an unconditional gate, not a documented rule, and a literal reading of the gate as written today does not license it. `02-codegraph.md` gains the identical clause `ADOPT-02` already carries: where `codegraph --version` already reports a version that satisfies this step's documented requirement, skip the installation action and its gate entirely and proceed straight to `codegraph init`, recording the reused version as evidence exactly as `ADOPT-02` already records a reused OpenSpec version. Where no usable version is already present, the installation action and its `[HUMAN APPROVAL REQUIRED]` gate are unchanged.

**14. `ADOPT-03`'s own pre-write approval gate (`01-prerequisites-and-install.md`, distinct from the checkpoint protocol's commit/push gates part 3 already covers) joins the same conditional treatment as `ADOPT-02`, `ADOPT-04`, `ADOPT-05`, and `ADOPT-13` (nineteenth revision).** `ADOPT-03`'s `Allowed modifications` is already a closed rule — a byte-for-byte mirror of `<SPECBOOT_SOURCE>/packages/specboot/template/{docs,ai-specs}` plus four named root symlinks, both endpoints pinned by parts 10 and 12 above — and the sixteenth revision (part 11) already requires the file counts and names presented at this gate to be the literal output of the same `find` commands the step's own `Validation` runs, executed before the gate rather than only after. Nothing about this step's output is discretionary once those two pieces are in place: where the pre-gate mechanical comparison shows the copy matches the closed rule exactly — every expected path present, nothing outside the two mirrored trees and the four symlinks — the gate auto-approves, recorded as evidence exactly as the mechanical count itself already is. Any deviation the comparison surfaces (a wrong source root, a miscount, an unexpected extra or missing path — precisely the three-times-wrong history this step's own evidence already records from a live pilot) requires the live gate exactly as today; the mechanism that would auto-approve is the same one that would have caught each of those three corrections; it does not weaken the check they document, it removes the live question only for the run that never needed one in the first place.

**15. `ADOPT-05B`'s single approval gate is split into the two decisions its own existing wording already names side by side without differentiating (nineteenth revision).** `03-client-permissions.md`'s gate reads "before creating the initial shared permission file **or** broadening it with a new command family" — one gate covering two different acts. Creating or merging the file strictly within the organization's already-reviewed generic baseline (this step's own Steps 1-4: copy or merge the baseline, reconcile only by removing entries outside the declared team environment matrix, add nothing beyond real project need already covered by that matrix) is deterministic content from a source reviewed once, elsewhere, in advance — the same category as `ADOPT-03`'s copy in part 14 above. That half auto-approves when the resulting file contains nothing beyond the baseline plus matrix-reconciliation removals. Broadening the file with a command family absent from that baseline remains a live decision, unchanged: it is new privilege nobody has reviewed, and the standing-authorization mechanism cannot certify a change to the very file that grants standing authorization for everything else — the permission file is the trust boundary the mechanism reads, not one more deterministic output alongside the others it already covers.

**16. Two gates were evaluated for the same treatment during this revision's review and kept unconditional — recorded here so the reasoning is not re-litigated the next time either is proposed (nineteenth revision).**

   - `ADOPT-06` (accept the adapted `docs/` content): the tenth revision's citation-check (part 4) proves a cited factual claim is real, never that it is *representative* of the repository's actual convention. `docs/backend-standards.md` and `docs/frontend-standards.md` synthesize a pattern across many instances of the codebase (naming conventions, layering, testing style); a citation only proves that one cited instance exists, not that it is the dominant pattern rather than a cherry-picked or unrepresentative example the code-graph traversal has no notion of "dominant across N instances" to rule out. This is a different failure mode than every case parts 6, 13, 14, and 15 cover, where the entire output is either literally copied bytes or a tool's own deterministic generation with no synthesis step. `api-spec.yml` and `data-model.md` are structurally verifiable this same way already, via the step's existing structural comparisons — but they are not separated into their own gate, because splitting one step's single checkpoint by file would add real mechanism for a benefit largely already captured by the citation-check on the content that most needs it. `ADOPT-06`'s gate stays live, unchanged.
   - `ADOPT-09`/`ADOPT-10` (replace an existing agent): re-read against its own wording — "before removing or **replacing an existing** agent" — this gate already fires only on an actual collision with content that predates the adoption, never on ordinary creation. A cold-start repository with no prior AI adoption, by construction (`ADOPTION-ENTRY-PROMPT.md` §0), has nothing to collide with, so this gate already produces zero live questions in the common case described throughout this revision's review. It exists for the deviation case — a target that turns out not to be as clean as the cold-start premise assumes — the same category of edge case `ADOPT-00`'s own pre-existing-`.gitignore` handling already documents from live pilot evidence. No text change needed.

*Correction, recorded against this same revision's own review:* `ADOPT-14` (`06-adapters-and-discovery.md`) was initially proposed, mid-conversation, as a fourth step needing this treatment. Re-reading the phase file directly shows this was mistaken: `ADOPT-14` is a pure read-only re-verification of what `ADOPT-13` just created (`find`/`readlink` checks only) and carries no `[HUMAN APPROVAL REQUIRED]` gate of its own to condition — the single gate governing creation of the adapters belongs to `ADOPT-13`, already made conditional by the eleventh revision (part 6). No task is added for `ADOPT-14`.
...
**The same launcher resumes an interruption at any point, and never re-runs the fresh-start spine over already-recorded answers (twentieth revision).** A live run hit an external usage-limit stop mid-`ADOPT-00` — not the designed stop-and-hand-off of step 6 above, an unplanned one, the kind that can land at any step for any cause (a session or usage limit, a crash, a closed terminal). The operator had nothing canonical to paste back in. Re-pasting the same launcher is not wrong in itself — it is still the sole pasted artifact, exactly as §9 requires — but `ADOPTION-ENTRY-PROMPT.md` as written would walk it straight through steps 3-6 again: re-asking `Q2`/`Q3` over a client selection already recorded, re-presenting a mutation inventory over state that may already be partly or fully provisioned. That is the corrective-patch failure this whole decision exists to remove, reached through a gap this amendment closes rather than a new one.

The fix stays inside the two-file mechanism rather than adding a third artifact: `ADOPTION-ENTRY-PROMPT.md` gains one check, placed immediately after source confirmation (§2) and before `Q2`/`Q3` are asked (§3): if `.specboot/adoption/ADOPTION-RUN-LOG.md` already exists and already records a client selection, this is a resume, not a cold start — steps 3-6 are skipped entirely, and the run proceeds to the resume procedure. That procedure (§8, "Step 7, in the fresh session") is generalized from a handoff-specific sequence to a general-purpose one: it already resumes from the durable manifest and run log, re-verifies source identity by checksum, and blocks on drift (D-S) — none of that changes. What generalizes is its step 5: **native skill discovery is attempted only when `ADOPT-00` has not yet recorded that evidence**; where it already has, that sub-step is skipped and the run continues directly from the step named in the run log's step-state table.

**This is deliberately distinct from the designed `ADOPT-00` handoff, not a replacement for it.** That handoff remains required exactly as stated above ("Where the initial session ends"): native discovery can only be evidenced by a session that started after the discovery entries existed, so the initial session still must stop and generate that exact prompt rather than being allowed to continue past provisioning. This amendment only extends what a *later*, *undesigned* interruption resumes into — an interruption after `ADOPT-00` has already passed reaches a repository where discovery evidence already exists, so there is no discovery left to attempt and nothing gained by pretending otherwise.

*Alternative rejected:* a dedicated `RESUME-PROMPT.template.md`. This is the literal third artifact §9 already refuses — it would duplicate the parameters and gates the launcher/entry-prompt pair already carries, and an operator with two pasteable files eventually pastes the wrong one, or the two drift against each other exactly as D-A and D-L already predict for any duplicated canonical content.

*Alternative rejected:* leaving the operator to compose an ad hoc resume instruction each time. Already tried, informally, in the run this amendment is evidenced by — it worked only because a human happened to know to point the session at the run log first; the whole reason this mechanism exists is that a corrective instruction improvised once is not a corrective instruction anyone can rely on twice.
