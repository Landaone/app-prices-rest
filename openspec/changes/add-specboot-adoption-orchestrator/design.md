...
*Alternative rejected, for parts 6-10 (composing with part 11 as a sixth instance of the same underlying principle — a live-verifiable claim beats a recalled one, whether the claim is an approval decision or a plain count):* leave these as documented rules and trust execution-time judgment to apply them correctly. Already tried, in the tenth revision's own text for parts 6 and 8 — stating the reasoning in `design.md` without a corresponding task or a tightened instruction in the phase file is exactly what let the gap survive one revision uncorrected.
