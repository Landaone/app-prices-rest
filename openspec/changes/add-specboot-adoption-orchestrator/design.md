...
The copy currently at `/Users/landaeta/repos/specboot` satisfies neither condition: it has **no `.git/`**, its guide differs from this repository's committed guide, and its kit is missing `ADOPTION-ENTRY-PROMPT.md`, `source-resolution.md`, and `client-autodiscovery.md` entirely. **It must not be used for the pilot.** That path is recorded here as *this change's* pilot evidence, in an OpenSpec planning artifact — it is not a canonical SpecBoot artifact, so D-R's no-hard-coded-path rule and the greps in tasks 3.9, 3.12, and 15.2 are unaffected, and the path must not appear in any canonical artifact.
...
### D-V. One canonical parameterized entry prompt, which invokes the contract rather than restating it
...
**The operating model the prompt makes explicit:** one canonical entry prompt; expected human responses at explicit approval gates; fresh-session handoff prompts **generated by the run** rather than composed by the operator; deterministic resume from the durable run log; and no improvised corrective prompt patches. Stating the model inside the prompt is what lets an operator recognize a patch as a defect rather than as normal use — an adoption that needed one has found a gap in the prompt, and that gap is an improvement proposal under D-U, not a habit.

**The cold-start contract: the prompt may not assume the state it exists to create.** At the moment the prompt is pasted, the target repository has **no SpecBoot files, no OpenSpec installation, no `/opsx:*` commands, no discoverable `specboot-adopt` skill, and no `.specboot/` directory**. Every one of those is an *output* of the adoption, so none may be a precondition of starting it. The earlier statement of this decision said the prompt "invokes the `specboot-adopt` skill", which reads as a skill invocation and is therefore wrong at the only moment the prompt is ever used. Three corrections follow:

1. **The prompt is ordinary prompt text.** It is pasted and run as-is — not a slash command, not a skill invocation, and requiring no command infrastructure, installed package, or prior configuration in the target repository.
2. **"Invokes the skill" means a direct read.** The initial session obtains the orchestration procedure by reading the *validated canonical* `ai-specs/skills/specboot-adopt/SKILL.md` at a **source-relative path** under the resolved source. This is a deliberate direct read and **explicitly not native skill discovery**; the prompt never assumes `specboot-adopt` is installed in, or discoverable from, the target repository. Because the initial session reads that file, source validation has to reach the file itself: under D-R a `specboot-adopt/` directory with no readable `SKILL.md` is an invalid source, not a technicality.
3. **Validation precedes loading as well as writing.** The prompt requests the canonical source, then validates the guide, the `specboot-adoption/` phase directory, and that `SKILL.md`, **before** it loads any orchestration instruction and before the first target-repository write. An invalid or absent source stops the run with **zero target-repository writes** — there is nothing to clean up because nothing was created.
...
**The prompt has to be executable from a virgin repository, end to end, and that is verified rather than assumed.** Being "complete in its parameters and gates" is a property of the prompt as *text*; being executable is a property of the prompt as *run*. The seven-step spine below is what an operator can actually perform in a repository with no tooling, and each step is a thing the prompt must state well enough to be followed without a corrective patch:

1. **Source question and validation** — ask for the canonical source; validate the guide, the phase directory, and a readable `SKILL.md`; refuse with zero writes on failure.
...
**Placeholders, never laboratory paths.** The prompt embeds no absolute path, no machine-specific location, and no pilot-specific value. Under D-R the source is runtime input; a prompt carrying one operator's path would hard-code into the most-copied artifact in the kit precisely what every other artifact is forbidden to record.
...
- **The entry prompt drifts from the contract it invokes** → It is verified for consistency at implementation and exercised verbatim by the pilot, where any corrective patch the operator had to improvise is recorded as a FAIL of the prompt and raised as an improvement proposal, not quietly typed and forgotten.
...
- **A laboratory path leaks into the most-copied file in the kit** → The prompt uses placeholders and runtime questions only, and a mechanical grep for absolute paths is part of its acceptance.
...
- **The pilot validates superseded instructions** → The pilot's canonical source must be Git-backed and byte-match the final committed guide, phase files, and skill (D-W). The stale non-Git copy that exists today is recorded as unusable for it rather than left to be discovered mid-run.
