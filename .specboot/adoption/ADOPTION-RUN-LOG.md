...
| `ADOPT-07` | `04-context-and-openspec.md` | PASS | 2026-08-25 — config.yaml: context + 19 rules + apply/archive guidance; negative control passed |
| `ADOPT-08` | `04-context-and-openspec.md` | PASS | 2026-08-25 — read-only verification, all checks PASS, doctor 0 warnings |
| `ADOPT-09` | `05-agents-and-skills.md` | PASS | 2026-08-25 — 3 repairs (representation-only), 1 agent created, selection corrected |
| `ADOPT-10` | `05-agents-and-skills.md` | PASS | 2026-08-25 — read-only; 4/4 agents parse, selection resolves live |
...
Claude Code / macOS  | yes | **ran, did NOT request permission (both attempts)** |
  **NOT APPLICABLE ON THIS CLIENT — row CLOSED on the alternative criterion, 2026-08-25T19:32:31Z.**
  **Attempt 1** (fresh session, handoff prompt verbatim): negative control `date` executed,
  exit 0, **no permission requested**. Session routed correctly, stopped at the control, ran
  none of the remaining eight commands as evidence, modified no files.
  **Attempt 2** (fresh session under the operator's own suggested remedy): identical outcome,
  with the file itself checked as part of the report — `.claude/settings.json` contains no
  `Bash(date...)` entry (the only "date" substring in the file sits inside
  `Bash(openspec validate:*)`), and the file declares no `defaultMode` or bypass that could
  explain it. It still ran unprompted. Session again routed correctly and modified no files.
  **Diagnosis, checked rather than assumed:** no configuration file explains the behaviour and
  none could be corrected — `~/.claude/settings.json` declares **no** `defaultMode`, holds 5
  allow entries and **no** broad `Bash` pattern; `~/.claude/settings.local.json` and
  `.claude/settings.local.json` are both **absent**. The auto-approval is a property of how
  sessions are launched in this environment, not of any settings file this adoption can point at
  or change. Two consistent attempts exhaust the retry.
  **This is NOT a FAIL of the permission file.** The client auto-approves outside the allowlist
  regardless of the file's contents, so the test cannot measure it in either direction — exactly
  the case this step defines `NOT APPLICABLE ON THIS CLIENT` for, and for which it directs a
  fall-through to the alternative criterion "rather than leaving the row open indefinitely".
  **Alternative criterion — all three conditions, and their evidential weight:**
    ...
    - Condition 2 (every canonical command still executes successfully when run, whether or not
      it prompts): **SATISFIED** — all 8 executed, every one **exit 0**:
      `openspec --version` (1.7.0), `openspec doctor --json` (218 B),
      `openspec context --json` (188 B), `openspec schemas` (162 B, lists `spec-driven`),
      `openspec templates` (587 B), `git status --short` (0 B),
      `git diff -- openspec/config.yaml` (0 B),
      `codegraph explore "PriceService"` (6953 B, 35 symbols across 4 files).
      The two 0-byte results are **meaningful results, not unexecuted commands**: both exit 0,
      the working tree was genuinely clean, and `openspec/config.yaml` had no unstaged change —
      so empty output is the correct answer here and is recorded as such, never inferred into a
      PASS. `git status --porcelain` confirmed **no file was modified** by the verification.
    - Condition 3 (**[HUMAN APPROVAL REQUIRED]** to close on this criterion): **GRANTED** by
      Landaone (operator) at 2026-08-25T19:32:31Z. Ruling and reason, recorded as the step demands: the
      operator first **declined** to close on this criterion and required a retry under a
      permission-enforcing mode; after that retry reproduced the same outcome and the
      configuration diagnosis showed no correctable cause, the operator approved closing on the
      alternative criterion **with an explicit recorded debt** (below) rather than leaving the
      adoption blocked.
  **EVIDENTIAL WEIGHT — stated plainly, not buried.** This is **weaker evidence than a PASS
  smoke test and is never to be treated as equivalent to one.** The criterion cannot itself
  distinguish a working permission file from having no permission file at all: it establishes
  that every canonical command is *covered on paper* and *runs*, and nothing about whether the
  allowlist is what caused it to run. The row is closed, not passed on its own merits.
  **RECORDED DEBT, at the operator's explicit instruction.** The smoke test **must be re-run in
  an environment that actually enforces project permissions**, and that re-run is owed **before
  `ADOPT-19`/`ADOPT-20`**. Until then no supported client/OS combination has a true PASS smoke
  test for this repository. Raised against the guide as improvement proposal 5.
...
Remaining limitations:
  ...
  3. **No supported client/OS combination has a true PASS smoke test.** The macOS row is closed
     on the alternative criterion — weaker evidence, explicitly labelled — after two fresh-session
     attempts both returned `NOT APPLICABLE ON THIS CLIENT`, and Linux and Windows are
     `PENDING EVIDENCE` for want of machines. The permission file is therefore **verified by
     inspection and by executability, but never observed actually gating anything.** Debt owed
     before `ADOPT-19`/`ADOPT-20`: re-run the smoke test where permissions are enforced.
  4. The claim "portability" is **not** made for this permission file. This step's own Rules
     require a fresh-session smoke test on every supported client/OS combination before claiming
     it, and that has not happened on any of the three declared rows.
Result: **PASS** — with its evidential basis stated exactly, not rounded up. Steps 1-5 complete:
        the file is authored, reconciled against the declared matrix, safety-checked and
        syntax-validated. The one available client/OS combination (Claude Code / macOS) is closed
        as `NOT APPLICABLE ON THIS CLIENT` on the **alternative criterion**, with the operator's
        recorded approval and the debt above; the two unavailable combinations are
        `PENDING EVIDENCE` with their reasons. This satisfies the step's stated PASS rule — every
        *available* combination resolved, every *unavailable* one recorded as pending — while
        making plain that no combination produced a true PASS smoke test.
```
...
```text
Prompt used:         The step's `CANONICAL CONSOLIDATED PROMPT`, executed as written, unmodified.
Config path:         `openspec/config.yaml` — the path that actually exists. The `.yml` alternative
                     the step allows for was not generated by this version and was not created.
OpenSpec version:    1.7.0.
Supported keys established from the installed version, NOT assumed from the guide:
                     `ProjectConfigSchema` in the installed package
                     (`dist/core/project-config.d.ts`) declares exactly `schema` (required
                     string), `context` (optional string), `rules` (optional record of artifact
                     ID to string array), `operations` (optional, with only `apply` and `archive`,
                     each carrying only `guidance`), and `store` (optional string).
                     `MAX_CONTEXT_SIZE` is 50 KB (`dist/core/project-config.js:173`).
                     **There is NO `agents` key in this version.** The prompt asks to "configure
                     agent selection using existing canonical agents" — with no key to carry it,
                     agent selection is expressed inside `context` prose instead, and the version
                     limitation is recorded here rather than a key being invented that the parser
                     would silently drop. Valid artifact IDs came from `openspec schemas`:
                     proposal, specs, design, tasks.
Files modified:      `openspec/config.yaml` only. `docs/`, agents, skills, source, tests and client
                     adapters were untouched, as the prompt requires.
Resolved context and rules:
                     `context` (2509 bytes, well under the 50 KB limit) — **pointers, not a
                     duplicate of the documentation**, as the prompt requires. It names
                     `docs/base-standards.md` as the primary source of project rules, references
                     the other five documents, states the stack and three-layer architecture,
                     records that `mvn` must be used rather than `./mvnw` (defect D7), and selects
                     `backend-developer` and `product-strategy-analyst` while stating that
                     `frontend-developer` **must not** be selected here because the repository has
                     no frontend for it to act on.
Rules:               4 artifact IDs, 19 rules total — proposal 4, specs 5, design 4, tasks 6.
                     Each is repository-specific rather than generic: the specs rules carry the
                     real `yyyy-MM-dd HH:mm:ss` space-separated format and the highest-priority-wins
                     selection rule; the design rules carry the single-underscore Flyway separator
                     trap; the tasks rules forbid a Playwright/E2E step and warn that changing the
                     seed INSERTs breaks `PriceControllerTest`.
Operations:          `apply.guidance` (3 entries) and `archive.guidance` (2). Configured **only
                     because this version supports them** — `OPERATION_IDS` in the installed
                     package is exactly `["apply", "archive"]`.
YAML quoting:        Every rule and guidance scalar containing a `:` is double-quoted, per the
                     prompt's explicit instruction and the `On failure` row for that exact hazard.
Warnings:            **zero.** `openspec doctor` exits 0 with no warning, error, or failure marker
                     in its output.
Corrections:         **One, self-caught.** An initial path-existence sweep reported
                     `docs/api-spec.yml.` and `docs/documentation-standards.md.` as missing. Both
                     were artefacts of the author's own extraction regex swallowing the sentence's
                     trailing full stop, not broken references. Re-run with trailing punctuation
                     stripped: all 13 referenced paths exist. Recorded because the raw first
                     result would otherwise read as two broken references.
Rules validation and its NEGATIVE CONTROL (the part `openspec doctor` cannot do):
                     The step is explicit that `doctor` does not read the `rules` block, so
                     `openspec instructions <artifact> --change <change>` — the call site that
                     actually consumes `rules` — was used instead, against a scratch change.
                     All four artifacts' rules appeared in the generated instructions verbatim.
                     **Negative control, as mandated:** the invalid artifact ID
                     `ThisArtifactIdDoesNotExist` was injected into `rules` and **WAS reported**,
                     verbatim: `Unknown artifact ID in rules: "ThisArtifactIdDoesNotExist". It
                     matches no artifact in any available schema. Known artifact IDs: design,
                     proposal, specs, tasks` — emitted for all four artifacts. `openspec doctor`
                     did **not** report it and still exited 0, which independently confirms the
                     step's own warning that doctor is necessary but not sufficient here.
                     **This corrected an earlier reading of mine.** Inspecting
                     `project-config.js` had suggested the parser accepts any key without
                     validating it, and therefore that the negative control would probably not be
                     reported. The parser does accept it — but the instruction-generation path
                     validates it, so the check is genuinely falsifiable. The empirical control is
                     what established this; the source reading alone would have recorded the wrong
                     conclusion.
                     The control was then removed and the config confirmed **byte-identical** to
                     its pre-injection state (`diff -q` against a copy taken before the
                     injection). The scratch change was deleted; `openspec/changes/` contains only
                     `archive/`, as it did before.
Result: PASS
```
...
```text
Read-only discipline: this step corrected nothing. The only failure it surfaced was in the
verification harness itself (see Corrections below), not in the configuration, and no
configuration entry was edited during this step.

Commands:            `openspec --version`; `openspec --help`; `openspec doctor`;
                     `openspec schemas`; `openspec instructions {proposal,specs,design,tasks}
                     --change <scratch>`; `openspec instructions {apply,archive} --change
                     <scratch>`; plus read-only filesystem and YAML inspection.
Exit codes:          `--version` **0** (prints 1.7.0); `--help` **0**; `doctor` **0**;
                     `schemas` **0**; all six `instructions` invocations **0**.
                     Harness sanity control, run first so that a PASS below cannot come from a
                     silently failing command: `openspec instructions proposal --change
                     no-such-change` exits **1**, while the same command against the real scratch
                     change exits **0**. The harness therefore distinguishes success from failure.
Schema:              `spec-driven` — resolves. `openspec schemas` lists it with its artifact chain
                     proposal → specs → design → tasks. **PASS**
Context:             13 referenced repository paths, every one confirmed to exist:
                     `docs/{base-standards.md,backend-standards.md,frontend-standards.md,
                     documentation-standards.md,api-spec.yml,data-model.md,development_guide.md}`,
                     `ai-specs/agents/` and its three agent files, `ai-specs/skills/`, and
                     `src/main/java/com/llandaeta/prices`. The context references all six intended
                     documentation files. **PASS**
Rules:               All four artifact rule sets resolve, each measured by exit code **and**
                     output size rather than by the absence of an error:
                     proposal exit 0 / 6920 B, specs exit 0 / 8776 B, design exit 0 / 6690 B,
                     tasks exit 0 / 6237 B — and **0** "Unknown artifact ID" reports across all
                     four. Sampled verbatim from the `tasks` output to confirm the rules are
                     actually injected rather than merely accepted: "Step 0 must be creating and
                     switching to a feature branch…", "…marked 'AGENT MUST EXECUTE'…", "Never
                     include a Playwright or end-to-end step…". **PASS**
Operations:          `apply.guidance` resolves (exit 0; "Permission to run a test never implies
                     its result is PASS" present in the output) and `archive.guidance` resolves
                     (exit 0; "rather than asserting that the change works" present). **PASS**
Agent references:    `ai-specs/agents/backend-developer.md`,
                     `ai-specs/agents/product-strategy-analyst.md` and
                     `ai-specs/agents/frontend-developer.md` all exist. The first two are selected;
                     the third is named in the context specifically to record that it must NOT be
                     selected for this repository. **PASS**
Skill references:    `ai-specs/skills/` contains exactly the 10 skills the context names —
                     adversarial-review, code-auditing, commit, enrich-us, explain, meta-prompt,
                     specboot-verify, update-docs, using-git-worktrees, writing-skills. **PASS**
Relative paths:      No `/Users/`, `/home/`, `C:\` or `$HOME` occurrence anywhere in
                     `openspec/config.yaml`. Every path is repository-relative. **PASS**
Warnings:            **zero.** `openspec doctor` exits 0 and its output contains no warning, error,
                     or failure marker. Recorded as necessary but **not sufficient**: doctor does
                     not read the `rules` block at all, which ADOPT-07's negative control
                     demonstrated directly. **PASS**
Corrections:         **One, in the verification harness, self-caught before it could produce a
                     false result.** A first pass ran the `instructions` checks with
                     `--change x`, a change that does not exist. Those commands failed outright and
                     produced no output — so the apply/archive checks reported FAIL, and, worse,
                     the "0 Unknown artifact ID reports" check reported **PASS on empty output**.
                     That is precisely the inference `00-conventions.md` forbids: empty output is
                     not evidence of success. The whole rules-and-operations verification was
                     re-run against a real scratch change, with exit codes and output byte counts
                     captured, and preceded by the sanity control described above. **No
                     configuration was changed** — the fault was in the measurement, not the thing
                     measured, so this remains a read-only step.
File modifications during this step: none. The scratch change created for the rules check was
                     deleted, and `openspec/changes/` contains only `archive/`. `git status`
                     shows `openspec/config.yaml` modified by ADOPT-07 and the run log; nothing
                     attributable to ADOPT-08.
Result: PASS
```
...
```text
Detected stacks and work types (from repository evidence, never a hardcoded filename):
                     Technology family: **JVM / Spring ecosystem** — Java 11 (`pom.xml:17`),
                     Spring Boot 2.4.5 (`pom.xml:8`), Spring MVC (`spring-boot-starter-web`),
                     Spring Data JPA/Hibernate, Flyway, H2, Lombok, Maven. Backend only: no
                     frontend stack is present anywhere (no `package.json`, no bundler, no static
                     or template resources).
                     Work types present: backend implementation/review/refactoring; scoping and
                     requirements analysis. **Not** present: frontend, browser, or E2E work.
Existing agents:     3, all inspected. Strict YAML frontmatter parsing was run on every one
                     **before** any adaptation, as the prompt requires:
                     - `backend-developer.md`       FAIL (ScannerError, line 3 col 654)
                     - `frontend-developer.md`      FAIL (ScannerError, line 3 col 528)
                     - `product-strategy-analyst.md` FAIL (ScannerError, line 3 col 326)
                     All three failed for **exactly one reason**, and it is the reason the prompt
                     anticipates: an unquoted `description` scalar containing YAML-sensitive text
                     — the embedded `Context:` pattern inside `<example>` blocks. Nothing was
                     semantically wrong with any of them.
Representation-only frontmatter repairs: **3 — and proven to be representation-only, not asserted
                     to be.** Each `description` was converted from a plain scalar to a literal
                     block scalar (`|-`). A before/after comparison against a pre-repair copy
                     confirms, per file: the **description text is byte-identical** after parsing,
                     the **body is byte-identical**, and `name`/`tools`/`model`/`color` are all
                     preserved. Re-validation after repair: all 3 parse strictly. Diff footprint
                     is `+1/-1` line per file — the split of one long line into two.
Agents preserved:    **3, none overwritten, none repurposed.**
                     - `backend-developer.md` — targets **TypeScript** backends with Domain-Driven
                       Design, Prisma, and Express. A different technology family, not a general
                       backend agent. Preserved unchanged beyond the representation repair.
                       Repurposing it into a Java agent is exactly what the prompt forbids.
                     - `frontend-developer.md` — targets React. Not applicable here, preserved.
                     - `product-strategy-analyst.md` — **technology-agnostic work type**, and
                       preserved *and reused*. The prompt is explicit that an agent must not be
                       rejected merely for not being a programming-stack agent; it is judged by
                       work type, and product/strategy analysis is a real work type here.
Agents created:      **1 — `ai-specs/agents/jvm-spring-backend-developer.md`.** Creation is
                     conditional and was justified by a real gap, not taken as a default step: no
                     existing validated agent covers the JVM/Spring technology family, and the
                     only backend agent present targets an unrelated stack.
                     Evidence it is **client-neutral**: frontmatter contains exactly `name` and
                     `description` and nothing else — no `model`, no `color`, no `tools`
                     allowlist; a scan of the whole file finds **zero** `mcp__*` identifiers and
                     zero model names. (The three pre-existing agents do carry `model`, `color`
                     and `tools` — they were left as found, since only representation repairs are
                     permitted on them.)
                     Evidence it is **domain-neutral**: a scan for this repository's identifiers
                     (`PriceEntity`, `PriceModel`, `PriceService`, `PriceController`,
                     `llandaeta`, `app-prices`, `35455`, `BRAND_ID`) and for the imported
                     template's LTI domain (`Candidate`, `Position`) returns **zero** matches, as
                     does a scan for hardcoded versions/packages of this repository
                     (`Spring Boot 2.4.5`, `Java 11`, `H2`, `com.llandaeta`). It is scoped to the
                     family — Java and Kotlin on Spring, Maven or Gradle, JPA/Hibernate, Flyway or
                     Liquibase — and explicitly excludes non-JVM backends and frontend work.
                     Evidence it **reads documentation at task time**: the agent's first section
                     instructs it to read the repository's standards, API contract, data-model and
                     development guide, and to prefer the build file over any document for real
                     dependency versions, rather than assuming any of them. Referenced paths are
                     generic (`CLAUDE.md`, `AGENTS.md`, `docs/`, `pom.xml`, `build.gradle`) —
                     no path that does not exist is asserted for any particular repository.
Agents modified:     the same 3, representation-only. No agent's meaning, scope, description text
                     or body changed.
OpenSpec selection:  **Corrected, and this is a correction of an error made earlier in this run.**
                     ADOPT-07 wrote a selection naming `ai-specs/agents/backend-developer.md` for
                     implementation work. That was wrong: the agent is a TypeScript/DDD/Prisma
                     agent, and ADOPT-07 selected it on the strength of its filename without
                     reading its description. ADOPT-09 is the step that inspects agents, and it
                     caught it.
                     The obsolete selection was **replaced, not appended to** (the prompt forbids
                     leaving a contradictory selection in place). The config now selects
                     `jvm-spring-backend-developer` for implementation and
                     `product-strategy-analyst` for scoping, and names `backend-developer` and
                     `frontend-developer` explicitly as MUST NOT be selected here, each with its
                     reason — so the exclusion reads as deliberate rather than as an oversight.
Corrections:         Two, both recorded above rather than silently applied: the three
                     representation-only frontmatter repairs, and the obsolete agent selection
                     inherited from ADOPT-07.
Approval gates:      **None was triggered, and none was owed.** This step's
                     `[HUMAN APPROVAL REQUIRED]` guards *removing or replacing an existing agent*.
                     No agent file was removed, and none was replaced — three were repaired in
                     representation only and one new file was added alongside them. Creating an
                     agent is not gated; it is gated only against being treated as a default step,
                     and the gap that justified it is recorded above.
Result: PASS
```
...
```text
Read-only discipline: this step changed nothing. Commands run were the step's own three, plus
strict YAML parsing and a scratch-change instruction render; the scratch change was removed.

Agent:               `find ai-specs/agents -maxdepth 1 -type f -name '*.md'` lists **4**:
                     `backend-developer.md`, `frontend-developer.md`,
                     `jvm-spring-backend-developer.md`, `product-strategy-analyst.md`.
                     The selected agent exists under `ai-specs/agents/`. **PASS**
Frontmatter:         Strict `yaml.safe_load` succeeds on **all 4 of 4**. **PASS**
Description:         Each description matches its technology family or work type, checked by
                     reading them rather than by filename: `jvm-spring-backend-developer` → JVM /
                     Spring (selected, and it is this repository's family);
                     `product-strategy-analyst` → technology-agnostic strategy work (selected);
                     `backend-developer` → TypeScript/DDD/Prisma/Express (correctly NOT selected);
                     `frontend-developer` → React (correctly NOT selected). **PASS**
Referenced docs:     Every `ai-specs/agents/*.md` path referenced from `openspec/config.yaml`
                     resolves on disk — all 4, including the two named only to record their
                     exclusion. The new agent references documentation generically and asserts no
                     path as necessarily existing. **PASS**
OpenSpec selection:  `grep -R "ai-specs/agents" openspec` returns 5 lines, all in
                     `openspec/config.yaml`, and OpenSpec selects an **existing canonical agent**.
                     Verified live rather than by reading the file: `openspec instructions
                     proposal --change <scratch>` exits **0** and its rendered output contains
                     `jvm-spring-backend-developer`, confirming the selection reaches the
                     generated context. The `MUST NOT be selected` exclusions also reach it, so a
                     consuming agent sees why the other two are off-limits. **PASS**
Unrelated agents preserved: All 3 pre-existing agents remain present. `git diff --stat` shows
                     `+1/-1` for each — the representation repair only — and the byte-identity
                     comparison of description text and body is recorded under ADOPT-09. No
                     unrelated agent was overwritten, narrowed, or repurposed. **PASS**
Project-specific details not unnecessarily duplicated: the new agent carries **no** stack
                     versions, package names, entity names, or build commands for this
                     repository; it instructs the reader to obtain them from `docs/` and the build
                     file at task time. **PASS**
Adapter not canonical: **PASS** — no client adapter was created or consulted as canonical in this
                     step. `git status --porcelain -- .claude .mcp.json` is empty, so the client
                     configuration was not touched at all. `ai-specs/agents/` remains the single
                     canonical, client-neutral source.
`openspec doctor`:   exit **0**, still clean after the configuration edit. **PASS**
Result: PASS
```
...
| 8 | `ADOPT-06` (single step, not a group) | n/a — not a group | `ADOPT-06` = **PASS**. 7 documents under `docs/` rewritten against the real repository, **+764 / -3154**, document set and file names unchanged. Two classes of template contamination removed: 130 stack keywords (TypeScript, React, `npm run`, PostgreSQL, Playwright, Node), and — the one a stack grep does **not** catch — `api-spec.yml` and `data-model.md` describing an entirely different product end to end (the "LTI" recruitment platform, `/candidates`, `/positions`, `/interviewflow`, 12 entities), both scoring 0 on that grep. 20 mechanical validations against `pom.xml`, the controller, the entity, the migration and `application.yaml`: **all PASS**. `api-spec.yml` parses as YAML, yielding exactly one path and two schemas. | Run log §`ADOPT-06` evidence block | **YES** — `ADOPT-06`'s `Allowed modifications` is `docs/`. All 7 staged content paths are under `docs/`, plus the always-permitted run log. The step's `Must preserve` list was checked **path by path, not by a single diff glance**: `src/`, `openspec/`, `ai-specs/agents/`, `ai-specs/skills/`, `.claude/`, `.mcp.json` and the root `CLAUDE.md` symlink are each individually unchanged. Nothing outside `docs/` was written. | YES — declared with the exact 8-path staged list before the gate | **LIVE approval, not auto** — Landaone, 2026-08-25T19:45:33Z. This step's gate is unconditional (`[HUMAN APPROVAL REQUIRED]` before accepting documentation changes) and was presented with the diffstat, both contamination classes with counts, all 8 defects with citations, the preservation check, and the validation results. The standing authorization covers the *commit*, never the step's own content gate; the two were kept distinct. | `docs/api-spec.yml` (M); `docs/backend-standards.md` (M); `docs/base-standards.md` (M); `docs/data-model.md` (M); `docs/development_guide.md` (M); `docs/documentation-standards.md` (M); `docs/frontend-standards.md` (M); `.specboot/adoption/ADOPTION-RUN-LOG.md` (M) — 8 paths | `1d61a14da5acf83a6ec0e7267676c96c6308e329` — filled after the commit; delta carries into checkpoint 9 | **UNCHANGED FROM THE `ADOPT-00` BASELINE — verdict: no automation triggered.** Re-probed read-only at 2026-08-25T19:45:33Z per §Amendment 1: `workflows=0 hooks=0 rulesets=0 branch-rules=[] keys=0 envs=0`. Identical to row 1's baseline. Fast-forward confirmed: `0/0` (behind/ahead), zero behind. | **PUSHED — auto-approved** under §Amendment 1. No force, no other branch, **no pull request**. | none new. **8 defects were documented, none fixed** — D1 (catch-all handler cannot bind what it advertises), D2 (unvalidated date parse, contract deliberately left unspecified in the OpenAPI rather than invented), D3 (`@Id` without `@GeneratedValue`), D4 (money as `double`, capped at 99.99 by `DECIMAL(4,2)`), D5 (filesystem-relative Flyway location), D6 (table-name case mismatch), D7 (`.mvn/` absent), D8 (doubled space in the 404 message, reproduced verbatim because it is what the service returns). These are repository defects for the product backlog, not improvement proposals against the guide. Proposals 1-5 remain `proposed`. |
| 9 | **GROUP: `ADOPT-07` + `ADOPT-08`** | **Guide-documented executed pair, and the justification is structural, not convenience.** `04-context-and-openspec.md` states in its own header that ADOPT-07's adapt action "is followed immediately by `ADOPT-08`'s read-only validation of the same configuration" and names it eligible for one checkpoint under `00-conventions.md`'s grouping rule — the same treatment the guide already gives ADOPT-09/10 and ADOPT-11/12. ADOPT-08 produces **no independently observable end state**: it writes nothing, and its only output is evidence about the artifact ADOPT-07 just wrote. Checkpointing them apart would create a commit whose entire content is a verification of the immediately preceding commit. "Fewer commits" is explicitly not the reason and is not claimed. | **ADOPT-07 = PASS.** `openspec/config.yaml` configured with `context` (2509 B of pointers, not duplicated documentation), 19 `rules` across all four artifact IDs, and `apply`/`archive` guidance. Supported keys were established from the **installed package's own schema**, not assumed: `schema`, `context`, `rules`, `operations`, `store` — and notably **no `agents` key exists in 1.7.0**, so agent selection is expressed in `context` prose rather than inventing a key the parser would drop. **ADOPT-08 = PASS.** Read-only re-verification: `--version`, `--help`, `doctor`, `schemas` and six `instructions` invocations all exit 0; 13 referenced paths all exist; all four rule sets resolve (6237-8776 B each, 0 unknown artifact IDs); both guidance blocks resolve; no absolute paths; `doctor` zero warnings. | Run log §`ADOPT-07` and §`ADOPT-08` evidence blocks | **YES** — the only staged content path is `openspec/config.yaml`, which is exactly `ADOPT-07`'s allowed-modification list ("only the path that exists" of `config.yaml`/`config.yml`), plus the always-permitted run log. `ADOPT-08` is read-only and contributes no staged path. **Both scratch changes were deleted before this checkpoint, as the step requires**: `openspec/changes/` contains only `archive/`, and nothing under `openspec/changes/` is staged. | YES — declared with the exact 2-path staged list before the gate | **auto: standing authorization** (Landaone, 2026-08-25T18:12:48Z); subset test YES; branch matches. `ADOPT-07`'s own gate is "none beyond the edit itself being reviewable", and `ADOPT-08` has no gate because it is read-only — so no live content gate was owed here, unlike ADOPT-06. | `openspec/config.yaml` (M); `.specboot/adoption/ADOPTION-RUN-LOG.md` (M) — 2 paths | `fe5d704f1a309c1c3ce7afa4552b446615699142` — filled after the commit; delta carries into checkpoint 10 | **UNCHANGED FROM THE `ADOPT-00` BASELINE — verdict: no automation triggered.** Re-probed read-only at 2026-08-25T19:51:53Z per §Amendment 1: `workflows=0 hooks=0 rulesets=0 branch-rules=[] keys=0 envs=0`. Identical to row 1's baseline. Fast-forward confirmed: `0/0` (behind/ahead), zero behind. | **PUSHED — auto-approved** under §Amendment 1. No force, no other branch, **no pull request**. | none new; proposals 1-5 remain `proposed`. **Two self-caught measurement faults are recorded in the evidence blocks rather than quietly fixed**, because both would have produced a wrong record: (a) a path-existence regex that swallowed sentence-final full stops and reported two live documents as missing; (b) an `instructions` harness run against a nonexistent change, whose commands failed outright — making the apply/archive checks FAIL and, more seriously, making a "0 unknown artifact IDs" check report **PASS on empty output**, the exact inference `00-conventions.md` forbids. The verification was re-run against a real scratch change with exit codes, byte counts, and a sanity control proving the harness can distinguish success from failure. |
| 10 | **GROUP: `ADOPT-09` + `ADOPT-10`** | **Guide-documented executed pair.** `05-agents-and-skills.md` states in its header that each adapt step "is followed immediately by its read-only validation step; they are executed as a pair", and `00-conventions.md` names ADOPT-09/ADOPT-10 as one of its two worked examples of a structurally justified group. ADOPT-10 writes nothing and produces no independently observable end state — its sole output is evidence about what ADOPT-09 just wrote. Not grouped for tidier history. | **ADOPT-09 = PASS.** All 3 pre-existing agents failed strict YAML parsing for exactly one anticipated reason — an unquoted `description` containing `Context:` — and were repaired **representation-only**, proven by byte-identity of description text and body against a pre-repair copy, with `name`/`tools`/`model`/`color` preserved. 1 agent created (`jvm-spring-backend-developer`) because no existing validated agent covers the JVM/Spring family; verified client-neutral (frontmatter is exactly `name`+`description`; zero `mcp__*` identifiers, zero model names) and domain-neutral (zero matches for this repo's identifiers **and** zero for the imported template's LTI domain). **ADOPT-10 = PASS.** 4/4 agents parse strictly; every config-referenced agent path resolves; the selection resolves **live** — `openspec instructions` exits 0 with `jvm-spring-backend-developer` in its rendered output. | Run log §`ADOPT-09` and §`ADOPT-10` evidence blocks | **YES** — 5 staged paths, all inside `ADOPT-09`'s allowlist: `ai-specs/agents/` limited to one creation plus representation-only repairs (4 paths), and the agent-selection portion of `openspec/config.yaml` (1 path), plus the always-permitted run log. `ADOPT-10` is read-only and stages nothing. **No client adapter was created or touched** — `git status --porcelain -- .claude .mcp.json` is empty, which the step requires explicitly. No scratch change is staged; `openspec/changes/` holds only `archive/`. | YES — declared with the exact 6-path staged list before the gate | **auto: standing authorization** (Landaone, 2026-08-25T18:12:48Z); subset test YES; branch matches. **`ADOPT-09`'s own `[HUMAN APPROVAL REQUIRED]` was not triggered and was not owed**: it guards *removing or replacing an existing agent*, and none was removed or replaced — three were repaired in representation only and one new file was added alongside them. Creating an agent is not gated, only forbidden as a *default* step, and the gap justifying it is recorded. | `ai-specs/agents/backend-developer.md` (M); `ai-specs/agents/frontend-developer.md` (M); `ai-specs/agents/product-strategy-analyst.md` (M); `ai-specs/agents/jvm-spring-backend-developer.md` (A); `openspec/config.yaml` (M); `.specboot/adoption/ADOPTION-RUN-LOG.md` (M) — 6 paths | PENDING-FILL — recorded immediately after the commit | **UNCHANGED FROM THE `ADOPT-00` BASELINE — verdict: no automation triggered.** Re-probed read-only at 2026-08-25T19:56:21Z per §Amendment 1: `workflows=0 hooks=0 rulesets=0 branch-rules=[] keys=0 envs=0`. Identical to row 1's baseline. Fast-forward confirmed: `0/0` (behind/ahead), zero behind. | **PUSHED — auto-approved** under §Amendment 1. No force, no other branch, **no pull request**. | none new against the guide. **One error made earlier in this run was found and corrected here:** checkpoint 9's `ADOPT-07` configuration selected `ai-specs/agents/backend-developer.md` for implementation work on the strength of its *filename*, without reading its description — it is a TypeScript/DDD/Prisma/Express agent, the wrong technology family for a Java/Spring repository. ADOPT-09 is the step that inspects agents, and it caught it. The obsolete selection was **replaced rather than appended to**, and the two unselected agents are now named with their reasons so the exclusion is visibly deliberate. Proposals 1-5 remain `proposed`. |
...
| 5 | `ADOPT-05B` | `specboot-adoption/03-client-permissions.md` | The step's smoke test can be defeated by the *client's* approval mode, and the guide gives no way to detect that before the run. This adoption spent two full fresh-session hand-offs to learn that the negative control would not gate — information that a one-line precheck could have surfaced in the session that authors the file. The `NOT APPLICABLE ON THIS CLIENT` path then routes to an alternative criterion the step itself calls weaker, so a mandatory step's PASS can rest on evidence that, by the step's own admission, cannot distinguish a working permission file from no permission file at all. Two concrete gaps: (a) there is no guidance on **how** to obtain a permission-enforcing session — the operator's own remedy (default/ask mode) failed here because the auto-approval came from how sessions are launched, not from any settings file, and the step offers nothing to check next; (b) closing on the alternative criterion creates a **standing debt** that the run log has no field for — this run had to invent one at the operator's instruction. Proposal: (i) add a precheck in the authoring session — run the negative control there first and, if it does not gate, say so immediately rather than after two hand-offs; (ii) document what actually determines gating (launch-time permission mode, not `defaultMode` in any settings file, which this run verified is absent at every level); (iii) add an explicit `Smoke-test debt owed` field to the run-log template so a row closed on the weaker criterion carries its obligation forward mechanically instead of by prose; and (iv) state whether a mandatory step may reach PASS with **no** combination smoke-tested, which is the situation this run is in. | `proposed` |
