...
Guide revision used:     8b7178b12e4766eeeac522987c31d9df65912bfc (originally
                          834ee535de598247318ed6beaccfb73793bfe064 at ADOPT-00's initial
                          provisioning; advanced twice during this run — see Drift check table
                          below)
```
...
| 2026-08-16, mid-run (operator-directed re-read of `00-conventions.md`, not a fresh session) | `.specboot/local/canonical-source-path` (same store, re-checksummed) | YES — unchanged, matched | YES — unchanged, matched | **NO** — source HEAD advanced `1271266d753c46c50e3b730c0b0284b19dc33648` → `8b7178b12e4766eeeac522987c31d9df65912bfc`, 3 intervening commits, source worktree still clean | Both checksums still match. The 3 intervening commits changed `00-conventions.md` and `01-prerequisites-and-install.md` (outside the checksum baseline) in ways **directly responsive to this run's own recorded mistakes**: (a) evidence discipline gained an explicit rule that gate counts must be the literal output of a mechanical command, never recalled — a direct response to this run's `ADOPT-03` miscounts (below); (b) the checkpoint protocol gained an explicit statement that advancing after a PASS checkpoint is never a live question — a direct response to this run asking "continue into ADOPT-01?" after `ADOPT-00`; (c) `ADOPT-03` gained an explicit statement that `<SPECBOOT_SOURCE>` reuses `ADOPT-00`'s resolved value rather than being re-derived — a response to this run's initial candidate-scanning of other worktrees. **A genuine unresolved tension surfaced by (c)**: the new text's own design-doc rationale (part 10) names `packages/specboot/template/` — the exact content already committed for `ADOPT-03` (`df654c2`) — as belonging to the deferred packaged-snapshot mode, not source-linked. A literal reading of the new phase-file text, applied to this run's actual sparse source, either resolves to nothing useful (empty on disk) or reproduces the already-rejected contaminated root content — neither of which matches what's committed. | Presented to the operator directly rather than silently reverting already-approved, already-pushed content or silently keeping it against the guide's new wording. Operator directed: keep `df654c2` as committed; log the conflict as an improvement proposal (see below) rather than resolve it unilaterally. |
...
| 8 | `ADOPT-03` (resumed, post-guide-update) | `01-prerequisites-and-install.md` `ADOPT-03`; `design.md` part 10 | **The guide's own fix for `<SPECBOOT_SOURCE>` ambiguity (design.md part 10, landed mid-run in response to this exact run's Improvement proposal #6) doesn't actually resolve for a sparse, contamination-carrying source like this run's.** The fix states `<SPECBOOT_SOURCE>` reuses `ADOPT-00`'s resolved value (`.specboot/local/canonical-source-path`) directly, and its own rationale explicitly names `packages/specboot/template/` — the content this run actually committed for `ADOPT-03` (`df654c2`), verified clean by direct `git ls-tree` inspection and approved by the operator after two rounds of correction — as belonging to the *deferred* packaged-snapshot mode, not source-linked. But a literal `cp -rn <SOURCE>/*` against this run's actual resolved source produces one of two outcomes, both already shown wrong: on-disk (sparse, only 3 materialized paths) it copies almost nothing; via its full tracked root tree (same mechanism this run used to read the sparse worktree elsewhere) it reproduces the exact contaminated content — self-referential `specboot-adopt` copy, `openspec-sync-specs`, `show-spec-working`, `sync-agent-symlinks`, extra `docs/openspec-tasks-mandatory-steps.md` — already identified and rejected earlier in this same run. The guide's fix correctly closes the "which worktree" ambiguity but leaves open "which subpath of that worktree," for the specific case where the resolved source's root itself carries non-template content (as this reference-experiment source evidently does, being simultaneously the guide's own development tree). Operator directed (2026-08-16): keep the already-committed `packages/specboot/template/` content rather than revert or re-resolve unilaterally; this proposal records the residual gap rather than acting on it. | proposed |
...
2026-08-16 / mid-run / re-read 00-conventions.md in full from source per operator instruction; two
  new conventions adopted going forward: (a) any count/enumeration presented at a gate must be the
  literal output of a mechanical command run immediately before, never recalled; (b) advancing to
  the next step after a checkpoint reaches PASS is never presented as a live "continue?" question —
  only that next step's own named gate, reached at its own point, stops execution / orchestrator
  (procedure, per updated canonical source) / the source's own three-commit advance was a direct
  response to this run's own recorded mistakes (ADOPT-03 miscounts, the post-ADOPT-00 continue
  question)

2026-08-16 / ADOPT-03 (resumed) / kept already-committed df654c2 content as-is rather than revert
  or re-resolve against the guide's new (but incomplete-for-this-source) <SPECBOOT_SOURCE> wording /
  Landaone / logged as Improvement proposal #8 instead; operator's explicit call, not the
