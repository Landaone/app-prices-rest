...
2. Search for mature ecosystem packages (external research — only with
   explicit user authorization)
3. Verify library health (commits, issues, activity — external research —
...
**Tools (detect stack and existing local tooling first; never install or download anything to run this check):**
- JavaScript/TypeScript: run `knip` only if it is already declared as a
  project dependency (present in `package.json`) and its executable is
  already available locally (e.g. in `node_modules/.bin`). Do not invoke it
  via `npx` in a way that could fetch the package on demand, and do not add
  it as a new dependency.
- Python: run `deadcode` only if it is already declared/configured for this
  repository (e.g. listed in `requirements*.txt`, `pyproject.toml`, or a
  lockfile) and already installed in the project's environment.
- Java/JVM and other stacks (Go, Rust, etc.): use only static-analysis
  plugins or commands already configured in the repository's build files
  (e.g. an existing Maven/Gradle plugin, linter, or IDE inspection profile).
- If no applicable tool is configured and locally available for the
  detected stack, fall back to manual verification: search for references
  and review existing compiler/build warnings. Do not install a new
  dependency or tool just to run this check.
